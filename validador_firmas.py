        ...
        with open(path, "rb") as f:
            ...
            if not signatures:
                return {
                    "status": "SIN FIRMA",
                    "details": [
                        "No se encontraron firmas digitales embebidas en el PDF."
                    ],
                    "file": path
                }
...
class App:
    ...
    def __init__(self):
        ...
        tk.Label(
            self.root,
            text="Validación local de firmas digitales en documentos PDF",
            font=("Segoe UI", 10)
        ).pack(pady=(0, 18))
        ...
        tk.Label(
            self.root,
            text=(
                "Nota: esta versión informa el resultado de la validación "
                "criptográfica disponible. "
                "La confianza administrativa de certificados debe configurarse "
                "según la política del organismo."
            ),
            font=("Segoe UI", 8),
            wraplength=820
        ).pack(pady=(0, 10))
    ...
    def process(self, paths):
        ...
        if len(results) == 1:
            ...
            self.status.config(
                text=f"{len(results)} DOCUMENTOS PROCESADOS"
            )
